...
from layer import SoftmaxAttention
...
class NeuralNetwork(nn.Module):
    ...
    def get_Matching_Map(self, bU, bE,bR,umask,emask,rmask):
        '''
        ...
        Hue, Heu = self._attention(bU, umask, bE, emask)
        Hur, Hru = self._attention(bU, umask, bR, rmask)
        Her, Hre = self._attention(bE, emask, bR, rmask)

        #utterance
        ue_input=torch.cat((bU,Hue,bU-Hue,bU*Hue),dim=-1)
        ur_input=torch.cat((bU,Hur,bU-Hur,bU*Hur),dim=-1)
        #evidence
        eu_input = torch.cat((bE, Heu, bE - Heu, bE * Heu),dim=-1)
        er_input = torch.cat((bE, Her, bE - Her, bE * Her),dim=-1)
        #response
        ru_input = torch.cat((bR, Hru, bR - Hru, bR * Hru),dim=-1)
        re_input = torch.cat((bR, Hre, bR - Hre, bR * Hre),dim=-1)
    ...
    def batch_att_cal(self,bertoutput,lenidx):
        ...
        r_arr = torch.zeros((batchsize, 200,768), dtype=torch.float32)
        c_mask = torch.zeros((batchsize, 256),dtype=torch.float32)
        e_mask = torch.zeros((batchsize, 250),dtype= torch.float32)
        r_mask = torch.zeros((batchsize, 200),dtype= torch.float32)
        ...
        for i in range(batchsize):
            ...
            r_arr[i, :lenidx[i][2] - lenidx[i][1]-1] = bertoutput[i,lenidx[i][1]+1:lenidx[i][2]][:200]
    ...
    def fit(self, train, dev, train_evi,dev_evi):############################여기가 메인임.
        ...
        if self.args.entire == True:
            ...
            self.optimizer = AdamW(optimizer_grouped_parameters, lr=self.args.learning_rate,
                                   correct_bias=False)  # weight_decay=self.args.l2_reg, correct_bias=False)
        ...
        for epoch in range(self.args.epochs):
            ...
            self.train()
            for i, data in (enumerate(tqdm(dataloader, mininterval=3600))):  # 원래 배치는 200
